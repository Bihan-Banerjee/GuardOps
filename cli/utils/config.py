...
DEFAULT_CONFIG: dict[str, Any] = {
    ...
    #       security:
    #         tools:
    #           owasp_zap: false   # relax DAST gate for staging
    #     prod:
    #       kubernetes:
    #         namespace: default
    #       security:
...
def get_env_config(config: dict, env: str) -> dict:
    ...
    """
    # Start from a shallow copy of the base config.
    merged = {k: v for k, v in config.items()}

    # Pull the environment block. Fall back to the DEFAULT_CONFIG environments
    # section so callers don't need to have every env block in their YAML.
    user_envs = config.get("environments", {})
    default_envs = DEFAULT_CONFIG.get("environments", {})

    # Merge: user env block takes priority over the built-in default.
    env_overrides: dict = {}
    if env in default_envs:
        env_overrides = {**default_envs[env]}
    if env in user_envs:
        # User's env block wins over our default env block.
    ...
    # Apply env overrides onto the merged config.
    for section_key, override_val in env_overrides.items():
        if section_key in merged and isinstance(merged[section_key], dict) and isinstance(override_val, dict):
            # Deep merge one level into the section (e.g., security.tools).
